...
| PR18 | ~~**Configure Sentry alert rules**~~ — ✅ Done 2026-09-11 via Sentry API. All 7 rules live in project `javascript-nextjs`, env `production`: ids `17445414` (Payment webhook fail), `17445415` (Reconciliation mismatch), `17445416` (Cron failure OR), `17445420` (IG signature spike 5-in-15m), `17445418` (Payment failure spike 10/hr), `17445419` (Daily digest), `17445413` (Error spike 20-in-15m). Email → `dmshiyam41@gmail.com`. Auth token rotated. | ✅ Done |
| PR19 | **Verify daily token refresh cron runs** — ⚠️ task doc said 03:00 UTC but `vercel.json` schedules `refresh-tokens` at **06:00 UTC** daily (03:00 UTC is actually `reconcile-payments`). Verification steps below. | 🟡 Code verified correct — **→ Venkat** to confirm via dashboard/Neon |
