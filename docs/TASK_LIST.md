...
| # | Task | Sub-tasks | Priority | Status |
|---|------|-----------|----------|--------|
| A8 | **Cross-browser + device QA** | | High | 🔶 In Progress — automated coverage in place, manual visual pass pending |
| | | 8.1 Full flow test on Chrome, Safari, Firefox (desktop) | | 🔶 In Progress — Playwright E2E suite (`tests/e2e/smoke.spec.ts`, 12 tests) covers landing / login / register / forgot / reset / dashboard-auth / admin-auth / legal pages. All 12 pass on chromium + firefox + webkit (36/36 green). Regression guards specifically protect commits `8d9eb6f` (Google button on /login) and `efd1f90` (friendly error copy, dual reveal-eye). Run with `pnpm test:e2e`. Visual pass still needs a human eye — checklist in `docs/QA_CHECKLIST.md` §4. |
| | | 8.2 Full flow test on mobile Chrome + mobile Safari | | 🔶 In Progress — Playwright emulates Pixel 7 (Chrome) and iPhone 14 (Safari) via `devices` config — same 12 tests pass on both (24/24 green). **Real iPhone Safari still required manually** for OAuth cookie behavior + ITP quirks (see checklist §3, "Google OAuth on iOS Safari"). |
| | | 8.3 Fix any layout/OAuth breakage found | | 🔶 In Progress — no functional regressions surfaced by the automated suite. Two UX/integrity issues found and fixed proactively via `efd1f90`: (1) raw "CredentialsSignin" leaking to users; (2) reset-password reveal-eye missing on Confirm field. A11y follow-ups (unwired `<label htmlFor>`) logged in `docs/QA_CHECKLIST.md` §6 — not blockers, surfaced when Playwright's `getByLabel` couldn't associate form pairs. Awaiting manual visual pass to flag CSS-specific bugs (backdrop-filter, gradients, scrollbar-hide). |
