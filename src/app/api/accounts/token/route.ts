...
import { getAccount, updateAccount, getAllAccounts, getAccountsWithExpiringTokens } from "@/lib/db";
import {
...
export async function POST(req: NextRequest) {
  ...
  let body: { action: string; account_id?: string; short_lived_token?: string; page_id?: string };
  try {
    body = await req.json();
  } catch {
  ...
  if (action === "exchange") {
    ...
    let expiresAt: string | null = computeExpiryDate(exchangeResult.expires_in ?? 5184000);
    ...
    if (account_id) {
      ...
      await updateAccount(account_id, { access_token: finalToken, token_expires_at: expiresAt });
    }
  ...
  if (action === "refresh") {
    ...
    await updateAccount(account_id, { access_token: result.access_token, token_expires_at: expiresAt });
  ...
  if (action === "refresh-all") {
    ...
    return NextResponse.json({ total_checked: expiringAccounts.length, results });
  }
  ...
  if (action === "debug") {
    ...
    return NextResponse.json({
      ...
      expires_at: info.expires_at === 0 ? "never" : new Date(info.expires_at * 1000).toISOString(),
      scopes: info.scopes,
...
export async function GET() {
  ...
  const statuses = accounts.map((acc) => {
    ...
    return { account_id: acc.id, username: acc.instagram_username, token_expires_at: expiresAt, status };
  });
