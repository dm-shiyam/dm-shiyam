...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (automation.user_id !== userId) return NextResponse.json({ error: "Forbidden" }, { status: 403 });
    ...
    if (schedule_days && !validDays.test(schedule_days)) {
      return NextResponse.json(
        { error: "schedule_days must be comma-separated 0–6 (0=Sun)" },
        { status: 400 }
      );
    }

    const updated = await updateAutomationSchedule(id, {
