          ...
          <div className="flex items-center gap-3">
            ...
            {stats && stats.accounts_connected > 0 && (
              <div className="hidden sm:flex items-center gap-1.5 text-xs text-blue-600">
                ...
                {stats.accounts_connected} account
                {stats.accounts_connected > 1 ? "s" : ""}
              </div>
            )}
...
function StatsGrid({ stats }: { stats: DashboardStats }) {
  const cards = [
    ...
    {
      label: "DMs Today",
      value: stats.dms_today,
      icon: Zap,
      color: "text-purple-600 bg-purple-50",
    },
    {
      label: "This Week",
      value: stats.dms_this_week,
      icon: BarChart3,
...
function TemplateSelector({
  ...
  onSelect: (template: Partial<Automation>) => void;
}) {
  const templates = [
    {
      icon: "🧲",
        ...
        <div className="space-y-3">
          {automations.map((auto) => (
            <AutomationCard
              ...
              onEdit={() =>
                setEditingId(editingId === auto.id ? null : auto.id)
              }
        ...
        <div className="mt-4 space-y-3 border-t border-gray-100 pt-4">
          ...
          {automation.ai_enabled && automation.ai_system_prompt && (
            <div>
              ...
              <p className="rounded-lg bg-violet-50 p-3 text-sm text-violet-700 whitespace-pre-wrap">
...
function AutomationForm({
  ...
  const [keywords, setKeywords] = useState(
    initial?.trigger_keywords || template?.trigger_keywords || ""
  );
  const [dmMessage, setDmMessage] = useState(
    initial?.dm_message || template?.dm_message || ""
  );
  const [replyComment, setReplyComment] = useState(
    initial?.reply_comment || template?.reply_comment || ""
  );
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <input
            type="text"
            className="input-field"
            placeholder='"INFO, LINK, GUIDE, SEND"'
            value={keywords}
